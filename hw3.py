...
# split data
x_train, x_test, y_train, y_test = train_test_split(x, y, random_state=rn, stratify=y)
...
kmeans_test = generate_kmeans_curve([2], kmeans_kwargs, x_test,y_test)
...
generate_silhouette_scores([2], 'kmeans', x_train, 1, 6)
generate_silhouette_scores([2], 'gmm', x_train, 1, 6)
#validate results
clusterer = KMeans(n_clusters=2, random_state=rn).fit(x_test)
...
# test clustering
generate_silhouette_scores([2], 'kmeans', x_test_pca, 0, 1)
...
add_cluster_label_nn("PCA",2,2,'diag',x_train_pca,x_test_pca,y_train,y_test)
# ICA
losses = []
kur = []
...
transformer = FastICA(n_components=10, random_state=rn)
X_transformed = transformer.fit_transform(x_train)
...
# cluster info + NN
add_cluster_label_nn("ICA",2,2,'diag',X_transformed,X_transformed_test,y_train,y_test)
##########################
...
for seed in iter_n:
    random_projection = SparseRandomProjection(n_components=10,random_state=seed)
    random_projection.fit(x_train)
...
plt.title("Randomized Projection Testing Performance By Random States - Bank Attrition")
...
plt.legend()
plt.title("Randomized Projection Performance with Cluster Label By Random States - Bank")
#####################################
## Truncated SVD
...
#plt.(xlabel='# of Component', ylabel='Cumulative Explained Variance Ratio')
tsvd = TruncatedSVD(10).fit(x_train)
...
add_cluster_label_nn("TSVD",2,2,'diag',x_transformed,x_test_transformed,y_train,y_test)